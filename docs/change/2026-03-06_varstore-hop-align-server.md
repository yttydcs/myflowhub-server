...
但 `docs/specs/varstore.md` 中对 `Major`、回程语义、`SourceID/TargetID`、以及若干规则点存在不一致/表述不清。

目标：把 `docs/specs/varstore.md` 与已确认的最终语义对齐，作为后续实现与联动升级的单一事实来源。

## 具体变更
更新 `docs/specs/varstore.md`，覆盖以下关键点：
...
- 回滚：回退 `docs/specs/varstore.md` 与本文件即可，不影响运行代码。
