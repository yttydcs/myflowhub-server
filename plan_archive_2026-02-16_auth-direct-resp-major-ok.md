# Plan Archive - Auth 直返 *_resp 统一为 MajorOKResp（PR10-Auth-DirectOKResp）

> 归档原因：本文件用于保留上一轮 workflow 的可执行计划，避免 `plan.md` 在新 workflow 中被覆盖后丢失上下文。
> 当前 workflow（PR11）会在 `plan.md` 中维护最新计划。
